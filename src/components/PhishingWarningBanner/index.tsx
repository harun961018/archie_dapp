...
import { Text, Flex, Box, CloseIcon, IconButton, useMatchBreakpoints } from '@archieneko/uikit'
...
const PhishingWarningBanner: React.FC = () => {
  ...
  const warningText = t("please make sure you're visiting https://app.archieneko.com - check the URL carefully.")
  const warningTextAsParts = warningText.split(/(https:\/\/app.archieneko.com)/g)
    ...
    <>
      ...
      {warningTextAsParts.map((text) => (
        <Text
          small
          as="span"
          bold={text === 'https://app.archieneko.com'}
          color={text === 'https://app.archieneko.com' ? '#FFFFFF' : '#BDC2C4'}
        >
          {text}
        </Text>
      ))}
