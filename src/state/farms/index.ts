...
export const fetchFarmsPublicDataAsync = createAsyncThunk<SerializedFarm[], number[]>(
  ...
  async (pids) => {
      // console.log('fetch farms public data');
      // console.log('pid: ', pids);
      // console.log('farms config: ', farmsConfig);
...
export const fetchFarmUserDataAsync = createAsyncThunk<FarmUserDataResponse[], { account: string; pids: number[] }>(
  ...
  async ({ account, pids }) => {
    ...
    const userFarmHarvest = await fetchFarmUserCanhavest(account, farmsToFetch)
    return userFarmAllowances.map((farmAllowance, index) => {
      return {
        ...
        canHarvest: userFarmHarvest[index],
      }
    })
  },
)
