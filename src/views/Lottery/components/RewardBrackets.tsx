...
import { Flex, Text } from '@archieneko/uikit'
...
const RewardBrackets: React.FC<RewardMatchesProps> = ({ lotteryNodeData, isHistoricRound }) => {
  ...
  useEffect(() => {
    if (lotteryNodeData) {
      const { treasuryFee, amountCollectedInArchieneko, rewardsBreakdown, countWinnersPerBracket } = lotteryNodeData

      const feeAsPercentage = new BigNumber(treasuryFee).div(100)
      const cakeToBurn = feeAsPercentage.div(100).times(new BigNumber(amountCollectedInArchieneko))
      const amountLessTreasuryFee = new BigNumber(amountCollectedInArchieneko).minus(cakeToBurn)
      setState({
        isLoading: false,
        cakeToBurn,
  ...
  return (
    <Wrapper>
      <Text color="black" fontSize="14px" mb="24px">
