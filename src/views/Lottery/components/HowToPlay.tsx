...
import { Box, Flex, Text, Heading, useMatchBreakpoints, Link, Image } from '@archieneko/uikit'
  ...
  return (
    <StyledStepCard width="100%">
      <StepCardInner height={['200px', '180px', null, '200px']}>
        <Text mb="16px" color="black" fontSize="12px" bold textAlign="right" textTransform="uppercase">
          {step.label}
        </Text>
        <Heading mb="16px" scale="lg" color="black">
          {step.title}
        </Heading>
        <Text color="black">{step.subtitle}</Text>
      </StepCardInner>
    </StyledStepCard>
  )
}
    ...
    <StyledStepCard width={['280px', '330px', '380px']}>
      <StepCardInner height="auto">
        ...
        <AllocationGrid>
          ...
          <Text textAlign="right" bold color="black">
            2%
          </Text>
  ...
  const steps: Step[] = [
    {
      ...
      title: t('Buy Tickets'),
      subtitle: t('Prices are set when the round starts, equal to 10 USD in Archieneko per ticket.'),
    },
    {
      label: t('Step %number%', { number: 2 }),
      title: t('Wait for the Draw'),
      subtitle: t('There are one or two draws every week: one every 4days.'),
    },
    {
    ...
    <Box width="100%">
      ...
      <GappedFlex flexDirection={['column', 'column', 'column', 'row']}>
        <Flex flex="2" flexDirection="column">
          <Heading mb="24px" scale="lg" color="black">
            {t('Winning Criteria')}
          </Heading>
          <Heading mb="24px" scale="md" color="black">
      ...
      <GappedFlex flexDirection={['column', 'column', 'column', 'row']}>
        <Flex flex="2" flexDirection="column">
          ...
          <Text color="black">{t('The prizes for each lottery round come from three sources:')}</Text>
          <Heading my="16px" scale="md" color="black">
            {t('Ticket Purchases')}
          </Heading>
          <BulletList>
            <li>
              <Text display="inline" color="black">
                {t('100% of the Archieneko paid by people buying tickets that round goes back into the prize pools.')}
              </Text>
            </li>
          </BulletList>
          <Heading my="16px" scale="md" color="black">
          ...
          <BulletList>
            <li>
              <Text display="inline" color="black">
                {t(
                  'After every round, if nobody wins in one of the prize brackets, the unclaimed Archieneko for that bracket rolls over into the next round and are redistributed among the prize pools.',
                )}
              </Text>
            </li>
          </BulletList>
          <Heading my="16px" scale="md" color="black">
            {t('Archieneko Injections')}
          </Heading>
          <BulletList>
            <li>
              <Text display="inline" color="black">
                {t(
                  'An average total of 35,000 Archieneko from the treasury is added to lottery rounds over the course of a week. This Archieneko is of course also included in rollovers! Read more in our guide to ',
                )}
                <InlineLink href="#" color="black">
                  {t('Archieneko Tokenomics')}
                </InlineLink>
              </Text>
            </li>
          </BulletList>
      ...
      {/* <Flex justifyContent="center" alignItems="center" flexDirection={['column', 'column', 'row']}>
        ...
        <Flex maxWidth="300px" flexDirection="column">
          ...
          <Text>
            ...
            <InlineLink href="#">
              {t('how to play the Archieneko LuckyStricke!')}
            </InlineLink>
          </Text>
        </Flex>
      </Flex> */}
