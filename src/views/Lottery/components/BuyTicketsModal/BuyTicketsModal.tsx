...
import { useLotteryV2Contract, useArchieneko } from 'hooks/useContract'
...
const BuyTicketsModal: React.FC<BuyTicketsModalProps> = ({ onDismiss }) => {
  ...
  const archienekoContract = useArchieneko()
  const { toastSuccess } = useToast();
  const { balance: userCake, fetchStatus } = useTokenBalance(tokens.archieneko.address)
  ...
  useEffect(() => {
    const getMaxPossiblePurchase = () => {
      const maxBalancePurchase = memoisedUserCake.div(priceTicketInArchieneko)
  ...
  useEffect(() => {
    ...
    const costBeforeDiscount = priceTicketInArchieneko.times(numberOfTicketsToBuy)
    const discountBeingApplied = costBeforeDiscount.minus(costAfterDiscount)
    setTicketCostBeforeDiscount(costBeforeDiscount.gt(0) ? getFullDisplayBalance(costBeforeDiscount, 9, 3) : '0')
    setTotalCost(costAfterDiscount.gt(0) ? getFullDisplayBalance(costAfterDiscount, 9, 3) : '0')
    setDiscountValue(discountBeingApplied.gt(0) ? getFullDisplayBalance(discountBeingApplied, 9, 3) : '0')
  }, [ticketsToBuy, priceTicketInArchieneko, discountDivisor, getTicketCostAfterDiscount])
  ...
  const { isApproving, isApproved, isConfirmed, isConfirming, handleApprove, handleConfirm } =
    useApproveConfirmTransaction({
      onRequiresApproval: async () => {
        try {
          const response = await archienekoContract.allowance(account, lotteryContract.address)
      ...
      onApprove: () => {
        return callWithGasPrice(archienekoContract, 'approve', [lotteryContract.address, ethers.constants.MaxUint256])
      },
  ...
  const getErrorMessage = () => {
    if (userNotEnoughCake) return t('Insufficient Archieneko balance')
  ...
  return (
    <StyledModal title={t('Buy Tickets')} onDismiss={onDismiss} headerBackground={theme.colors.gradients.cardHeader}>
      ...
      <BalanceInput
        ...
        currencyValue={
          cakePriceBusd.gt(0) &&
          `~${ticketsToBuy ? getFullDisplayBalance(priceTicketInArchieneko.times(new BigNumber(ticketsToBuy)), 9) : '0.00'} Archieneko`
        }
      />
      ...
      <Flex flexDirection="column">
        <Flex mb="8px" justifyContent="space-between">
          ...
          <Text color="textSubtle" fontSize="14px">
            {priceTicketInArchieneko && getFullDisplayBalance(priceTicketInArchieneko.times(ticketsToBuy || 0), 9)} Archieneko
          </Text>
        </Flex>
        <Flex mb="8px" justifyContent="space-between">
          <Flex>
        ...
        <Text mt="24px" fontSize="12px" color="textSubtle">
          {t(
            '"Buy Instantly" chooses random numbers, with no duplicates among your tickets. Prices are set before each round starts, equal to $10 at that time. Purchases are final.',
          )}
        </Text>
      </Flex>
    </StyledModal>
