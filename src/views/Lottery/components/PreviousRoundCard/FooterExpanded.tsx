...
import { Flex, Skeleton, Heading, Box, Text } from '@archieneko/uikit'
...
const PreviousRoundCardFooter: React.FC<{ lotteryNodeData: LotteryRound; lotteryId: string }> = ({
  ...
  if (lotteryNodeData) {
    const { amountCollectedInArchieneko } = lotteryNodeData
    prizeInBusd = amountCollectedInArchieneko.times(cakePriceBusd)
  }
    ...
    return (
      <>
        {prizeInBusd.isNaN() ? (
          ...
          <Heading scale="xl" lineHeight="1" color="black">
        ...
        {prizeInBusd.isNaN() ? (
          ...
          <Balance
            ...
            color="black"
            unit=" ARCHIE"
            value={getBalanceNumber(lotteryNodeData?.amountCollectedInArchieneko)}
            decimals={0}
          />
        )}
      </>
  ...
  return (
    <NextDrawWrapper>
      <Flex mr="24px" flexDirection="column" justifyContent="space-between">
        <Box>
          <Heading color="black">{t('Prize pot')}</Heading>
          {getPrizeBalances()}
        </Box>
        <Box mb="24px">
          <Flex>
            <Text color="black" fontSize="14px" display="inline">
